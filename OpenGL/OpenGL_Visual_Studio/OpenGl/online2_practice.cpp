...
void initcp(){
	if (model){
		for (int i = 0; i < cpidx; i++){
			cp[i].x += (model1[i].x - cp[i].x)/1000;
			cp[i].y += (model1[i].y - cp[i].y)/1000;
		}
	}
	else{
		for (int i = 0; i < cpidx; i++){
			cp[i].x += (model2[i].x - cp[i].x)/1000;
			cp[i].y += (model2[i].y - cp[i].y)/1000;
		}
	}
}
...
void init(){
	...
	file0.open("model0.txt");
	file1.open("model1.txt");
	int n;
	file1 >> n;
	file0 >> n;
	cout << n << endl;
	cpidx = n;
	for (int i = 0; i < n; i++){
		file1 >> model1[i].x >> model1[i].y;
		file0 >> model2[i].x >> model2[i].y;
	}
	initcp();
	//clear the screen
